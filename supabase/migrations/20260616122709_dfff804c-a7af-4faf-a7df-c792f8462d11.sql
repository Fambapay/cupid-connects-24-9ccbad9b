...
CREATE OR REPLACE FUNCTION public.get_unread_chats_count()
RETURNS int LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  WITH me AS (SELECT auth.uid() AS uid),
  my_matches AS (
    SELECT id FROM public.matches
    WHERE user_a = (SELECT uid FROM me) OR user_b = (SELECT uid FROM me)
  ),
  reads AS (
    SELECT match_id, last_read_at FROM public.match_reads WHERE user_id = (SELECT uid FROM me)
  )
  SELECT COUNT(DISTINCT msg.match_id)::int
  FROM public.messages msg
  JOIN my_matches mm ON mm.id = msg.match_id
  LEFT JOIN reads r ON r.match_id = msg.match_id
  WHERE msg.sender_id <> (SELECT uid FROM me)
