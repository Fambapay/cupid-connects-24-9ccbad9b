CREATE OR REPLACE FUNCTION public.get_discovery_feed(_filters jsonb DEFAULT '{}'::jsonb, _viewer_lat double precision DEFAULT NULL::double precision, _viewer_lng double precision DEFAULT NULL::double precision, _limit integer DEFAULT 100)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  base AS (
    ...
    WHERE p.onboarding_completed = true
      ...
      AND (v_height_min IS NULL OR p.height_cm IS NULL OR p.height_cm >= v_height_min)
      AND (v_height_max IS NULL OR p.height_cm IS NULL OR p.height_cm <= v_height_max)
  ),
  with_distance AS (
    SELECT * FROM base
