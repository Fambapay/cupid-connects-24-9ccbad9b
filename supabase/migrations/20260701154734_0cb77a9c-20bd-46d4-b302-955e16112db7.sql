...
BEGIN
  ...
  -- Reveal identity ONLY to paying plus/elite subscribers whose subscription
  -- is active (not trialing, not expired). Trial users see a teaser only.
  ...
  WITH pending AS (
    ...
    WHERE s.swiped_id = _viewer
      ...
      -- Exclude those the viewer already responded to.
      AND NOT EXISTS (
  ...
  FROM (
    SELECT
      CASE WHEN _reveal THEN p.swiper_id::text
           ELSE 'hidden-' || encode(digest(p.swiper_id::text || _viewer::text, 'sha256'), 'hex')
      END AS id,
      ...
      -- Teaser gets a photo path too (client signs it tiny/pixelated).
